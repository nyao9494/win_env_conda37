...
export class GitExtension {
    ...
    async checkout(options) {
        ...
        const data = await this._taskHandler.execute('git:checkout', async () => {
            ...
            if (body.checkout_branch) {
                const changes = await this._changedFiles(this._currentBranch.name, body.branchname);
                (_a = changes.files) === null || _a === void 0 ? void 0 : _a.forEach(file => this._revertFile(file));
            }
            else {
    ...
    async resetToCommit(hash = '') {
        ...
        await this._taskHandler.execute('git:reset:hard', async () => {
            ...
            (_a = files) === null || _a === void 0 ? void 0 : _a.forEach(file => {
                this._revertFile(file);
            });
        });
        await this.refreshBranch();
    ...
    async showPrefix(path) {
        return await this._taskHandler.execute('git:fetch:prefix_path', async () => {
    ...
    async showTopLevel(path) {
        return await this._taskHandler.execute('git:fetch:top_level_path', async () => {
